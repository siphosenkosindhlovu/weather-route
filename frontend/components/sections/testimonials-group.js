...
const TestimonialsGroup = ({ data }) => {
  ...
  return (
    <section className="text-center text-lg bg-green-primary pt-12 pb-16">
      <div className="container">

        <h2 className="title mb-12 text-white">{data.title}</h2>
      ...
      <div className="max-w-5xl w-8/12 sm:w-8/12 bg-white shadow-md sm:shadow-xl mx-auto flex flex-col sm:flex-row mt-10 text-left">
          ...
          >
            {
              data.testimonials.map((item, i) => (
                <SwiperSlide key={i} style={{ height: 'auto' }}>
                  <div className="bg-white flex flex-col gap-4 max-w-xs py-5 px-3 text-left h-full">
                    {item.rating && (
                      <div className="flex gap-1">
                        {(new Array(mapLetterToRating[item.rating])).fill(<BsStarFill className="text-yellow-400" />)}
                      </div>)}
                    <div className="font-medium text-sm">
                      {item.text}
                    </div>
                    <div className="text-sm italic text-right mt-auto">
                      - {item.authorName}
                    </div>
                  </div>
                </SwiperSlide>
        ...
        {/* Logos list */}
        {/* {data.logos && <div className="flex flex-row flex-wrap items-center gap-6 sm:gap-20 justify-center mt-10 px-6 sm:px-0 ">
          {data.logos.map((logo) => (
